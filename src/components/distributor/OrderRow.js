...
const OrderRow = ({ item, purchaseOrderId }) => {
  ...
  const setAction = state => {
    switch (state) {
      ...
      case "can not be full filled":
        return (
          <span>
            You do not have sufficient quantity to full-fill this order
          </span>
        );
      case "completed":
  ...
  const handleSale = () => {
    let [orderId, puid] = parsePurchaseOrderId(item.productId);
    let batchObject = {
      totalUnitsForSale: item.orderedQuantity,
      sentToRetailerOn: new Date().toLocaleString(),
      productUnitId: convertToHex(puid),
  ...
  return (
    <>
      <tr>
        ...
        <td>{item.price}</td>
        <td>{parseFloat(item.price) * parseFloat(item.orderedQuantity)}</td>
        <td>{setAction(itemState)}</td>
      </tr>
